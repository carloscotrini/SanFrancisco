...
class Ants(Game):
    def __init__(self, options=None):
        ...
        self.carry_food = options.get ('carry_food', 10)
    ...
    def render_changes(self, player):
        ...
        for update in updates:
            ...
            if v[row][col] or (ilk == 'd' and update[3] == player):
                ...
                if ilk in ['a', 'd', 'h']:
                    ...
                    #   assign the enemy the next index
                    if self.switch[player][update[3]] is None:
                        self.switch[player][update[3]] = self.num_players - self.switch[player].count(None)
                    update[3] = self.switch[player][update[3]]
    ...
    def get_state_changes(self):
        ...
        # current ants
        changes.extend(sorted(
            ['a', ant.loc[0], ant.loc[1], ant.owner, ant.food_amount]
            for ant in self.current_ants.values()
        ))
        # current food
        changes.extend(sorted(
    ...
    def do_gather(self):
        """Switch between original game logic and food carrying"""

        if self.carry_food == 0:
            self.do_gather_orig()
        else:
            self.do_gather_carry()
            self.do_deliver_food()

    def do_gather_orig(self):
        """ Gather food
    ...
    def do_gather_carry(self):
        for f_loc in list (self.current_food.keys()):
            nearby_ants = self.nearby_ants(f_loc, self.spawnradius)
            nearby_players = set (ant.owner for ant in nearby_ants)
            if len(nearby_players) == 1:
                for ant in nearby_ants:
                    if ant.food_amount < self.carry_food:
                        ant.food_amount += 1
                        self.remove_food (f_loc)
                        break
    ...
    def do_deliver_food(self):
        for h_loc, hill in self.hills.items():
            nearby_ants = self.nearby_ants (h_loc, self.spawnradius)
            if h_loc in self.current_ants:
                nearby_ants.append (self.current_ants[h_loc])
            for ant in nearby_ants:
                if hill.owner == ant.owner and ant.food_amount > 0:
                    self.hive_food[hill.owner] += ant.food_amount
                    ant.food_amount = 0
