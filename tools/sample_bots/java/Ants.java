...
public class Ants {
	...
	private Map<Tile, Ilk> antList = new HashMap<Tile, Ilk>();
        private Map<Tile, Integer> foodAmount = new HashMap<Tile, Integer>();
	...
	private boolean update(List<String> data) {
		...
		for (String line : data) {
			...
			if (tokens.length > 2) {
				...
				this.foodAmount.put (new Tile(row, col), Integer.parseInt(tokens[4]));
	...
	public Set<Tile> enemyAnts() {
        ...
        public int getFoodAmount (Tile t) {
	        return this.foodAmount.get(t);
        }
